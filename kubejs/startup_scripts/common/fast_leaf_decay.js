...
global.plannedUpdates = [];
global.scheduledUpdates = [];
...
ForgeEvents.onEvent("net.minecraftforge.event.TickEvent$ServerTickEvent", (event) => {
    if (event.side == "SERVER" && event.phase == "END") {
        if (global.plannedUpdates.length > 0) {
            Array.prototype.push.apply(global.scheduledUpdates, global.plannedUpdates);
            global.plannedUpdates = [];
        }

        let newScheduledUpdates = [];

        for (let blockUpdate of global.scheduledUpdates) {
            blockUpdate.tick--;
        ...
        global.scheduledUpdates = newScheduledUpdates;
    }
});
